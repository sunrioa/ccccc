...
document.addEventListener('click',e=>{
 ...
 case'retry':guarded(async()=>{await api('/jobs',{method:'POST',body:JSON.stringify({kind:j.kind,device_id:j.device_id,provider:j.provider,project:j.project,snapshot_id:j.snapshot_id,new_project:j.new_project,folder:j.folder})});await refresh();toast('已重新排队')});break;
...
$('#upload-button').onclick=()=>$('#upload-file').click();$('#upload-file').onchange=()=>guarded(async()=>{const f=$('#upload-file').files[0];if(!f)return;if(f.size>2*1024**3)throw Error('压缩包不能超过 2 GiB');toast('正在上传并校验压缩包…');await api('/snapshots/upload',{method:'POST',body:f});$('#upload-file').value='';await refresh();toast('压缩包已导入快照库')});
