...
function projectLinks(device,project){return (state.bindings||[]).flatMap(b=>b.a.device_id===device&&b.a.project===project?[{id:b.id,local:b.a,other:b.b}]:b.b.device_id===device&&b.b.project===project?[{id:b.id,local:b.b,other:b.a}]:[])}
function sameProjectPath(device,a,b){const win=state.devices.find(d=>d.id===device)?.os?.startsWith('windows');const normalize=p=>{p=String(p||'').replace(/\\/g,'/').replace(/\/$/,'');return win?p.toLowerCase():p};return normalize(a)===normalize(b)}
function bindingDestination(device,project,path,target){return projectLinks(device,project).find(link=>sameProjectPath(device,link.local.path,path)&&(!target||link.other.device_id===target)&&state.devices.some(d=>d.id===link.other.device_id&&d.projects.some(p=>p.key===link.other.project&&!p.missing&&sameProjectPath(d.id,p.path,link.other.path))))?.other}
function bindingSummary(device,project){const links=projectLinks(device,project);return `<div class="project-binding"><button class="text-button" data-action="bind-project" data-device="${device}" data-project="${esc(project)}">${links.length?'管理绑定（'+links.length+'）':'绑定另一台电脑的项目'}</button>${links.map(l=>`<small class="muted">↔ ${esc(deviceName(l.other.device_id))} · ${esc(l.other.path)}</small>`).join('')}</div>`}
function bindingModal(device,project){
 const source=state.devices.find(d=>d.id===device)?.projects.find(p=>p.key===project);if(!source){toast('项目已变化，请刷新后重试');return}
 const links=projectLinks(device,project),targets=state.devices.filter(d=>d.id!==device);
 modal(`<h2>绑定对应项目</h2><p>${esc(deviceName(device))} · <code>${esc(source.path)}</code></p><p class="notice">确认两端属于同一个项目后再绑定。绑定会双向生效，后续恢复自动选中对方项目；不会上传、合并或写入会话。</p>${links.length?`<h3>已保存的绑定</h3>${links.map(l=>`<div class="binding-row"><p>${esc(deviceName(l.other.device_id))} · <code>${esc(l.other.path)}</code></p><button class="text-button danger" data-action="unbind-project" data-id="${l.id}">解除绑定</button></div>`).join('')}`:''}${targets.length?`<form id="binding-form"><label>对应设备<select id="binding-device">${targets.map(d=>`<option value="${d.id}">${esc(d.name)}</option>`).join('')}</select></label><label>查找对应项目<input id="binding-search" type="search" placeholder="输入项目名或路径"></label><label>对应项目<select id="binding-project" required></select></label><p class="muted" id="binding-hint"></p><div class="dialog-actions"><button class="primary" id="save-binding">确认并保存绑定</button></div></form>`:'<p class="muted">请先添加并连接另一台设备。</p>'}`);
 if(!targets.length)return;
 const update=()=>{const d=state.devices.find(d=>d.id===$('#binding-device').value),query=$('#binding-search').value.trim().toLowerCase();const previous=$('#binding-project').value;const ps=(d?.projects||[]).filter(p=>!p.missing&&p.path.toLowerCase().includes(query)).sort((a,b)=>Number(shortPath(b.path).toLowerCase()===shortPath(source.path).toLowerCase())-Number(shortPath(a.path).toLowerCase()===shortPath(source.path).toLowerCase()));$('#binding-project').innerHTML=ps.map(p=>`<option value="${esc(p.key)}">${esc(p.path)}${shortPath(p.path).toLowerCase()===shortPath(source.path).toLowerCase()?'（同名候选，需确认）':''}</option>`).join('');if(ps.some(p=>p.key===previous))$('#binding-project').value=previous;$('#save-binding').disabled=!ps.length;$('#binding-hint').textContent=ps.length?'找到 '+ps.length+' 个项目。同名不代表同一项目，请核对完整路径。':'没有可选项目。目标还没有项目时，先通过恢复预览新建，再在预览页保存绑定。'};
 update();$('#binding-device').onchange=update;$('#binding-search').oninput=update;
 $('#binding-form').onsubmit=e=>{e.preventDefault();guarded(async()=>{await api('/bindings',{method:'POST',body:JSON.stringify({source_device:device,source_project:project,target_device:$('#binding-device').value,target_project:$('#binding-project').value})});closeModal();await refresh();toast('已保存双向绑定；恢复时将自动选择对应项目')})};
}
function previewModal(sid){
 ...
 modal(`<h2>恢复 ${esc(base)}</h2><p class="muted">${names[b.provider]} · ${b.sessions} 个记录 · ${date(b.created)}</p><form id="preview-form"><label>目标设备<select id="job-device">${deviceOptions()}</select></label><label>导入位置<select id="job-project" required></select></label><label id="new-folder-label" hidden>新项目文件夹名称<input id="new-folder" value="${esc(folder)}" maxlength="100"></label><p class="muted" id="import-location"></p><p class="notice" id="binding-selection" hidden></p><div class="notice">目标电脑没有此项目或会话也可以导入。新建时先创建空目录和项目映射，会话仍需预览后确认写入。项目代码和依赖请另行准备。请关闭目标电脑上的 ${names[b.provider]}。</div><div class="dialog-actions"><button class="primary">生成恢复预览</button></div></form>`);
 const updateLocation=()=>{const d=state.devices.find(x=>x.id===$('#job-device').value);const isNew=$('#job-project').value==='__new';$('#new-folder-label').hidden=!isNew;$('#new-folder').required=isNew;$('#import-location').textContent=isNew?'将导入到 '+d.import_root+(d.os?.startsWith('windows')?'\\':'/')+$('#new-folder').value:'将映射到所选项目的本机路径。';const linked=bindingDestination(b.device_id,b.project,b.source_path,d.id);const hasLink=projectLinks(b.device_id,b.project).some(l=>l.other.device_id===d.id);$('#binding-selection').hidden=!linked&&!hasLink;$('#binding-selection').textContent=linked?($('#job-project').value===linked.project?'已根据保存的绑定选中对应项目。':'本次手动选择了其他项目，原绑定保持不变。'):'此设备已有绑定，但对应路径已变化或暂不可用。请核对项目，必要时解除并重新绑定。'};
 const update=()=>{const d=state.devices.find(x=>x.id===$('#job-device').value);const ps=(d?.projects||[]).filter(p=>!p.missing);$('#job-project').innerHTML=(d?.import_root?'<option value="__new">＋ 在本机新建项目（无需已有会话）</option>':'')+ps.map(p=>`<option value="${esc(p.key)}">${esc(p.path)}</option>`).join('');const linked=bindingDestination(b.device_id,b.project,b.source_path,d.id);if(linked)$('#job-project').value=linked.project;else if(ps.some(p=>p.key===b.project))$('#job-project').value=b.project;updateLocation()};
 const paired=bindingDestination(b.device_id,b.project,b.source_path);const other=(paired&&state.devices.find(d=>d.id===paired.device_id))||state.devices.find(d=>d.id!==b.device_id&&online(d))||state.devices.find(d=>d.id!==b.device_id);if(other)$('#job-device').value=other.id;
...
function planModal(j){
 const p=j.result;const canBind=p.can_apply&&j.source&&state.devices.some(d=>d.id===j.source.device_id)&&j.source.device_id!==j.device_id;modal(`<h2>恢复预览</h2><p class="muted">目标：${esc(deviceName(j.device_id))} · ${esc(p.target)}</p>${p.expanded_bytes?`<p class="muted">展开约 ${bytes(p.expanded_bytes)}，建议预留至少 ${bytes(p.expanded_bytes*3)} 的暂存和备份空间。</p>`:''}${p.warnings?.length?`<div class="notice">${p.warnings.map(esc).join('<br>')}</div>`:''}<div class="plan-list">${(p.changes||[]).map(c=>`<div class="plan-line ${esc(c.action)}"><b>${esc(actions[c.action]||c.action)}</b>${esc(c.path)}</div>`).join('')}</div>${p.can_apply?'<p class="notice">预览已保存在目标电脑；临时服务器包可能已清理，不影响确认恢复。写入前再次核对文件并备份，若本机有新变化会拒绝覆盖。</p>':'<p class="error">存在冲突或不支持的记录，本机原文件未变。临时包会保留到到期，请处理冲突后重新预览。</p>'}${canBind?`<div class="notice"><p>来源：${esc(deviceName(j.source.device_id))} · ${esc(j.source.path)}<br>目标：${esc(deviceName(j.device_id))} · ${esc(p.target)}</p><button class="secondary" id="bind-preview">确认两端是同一项目，保存绑定</button></div>`:''}<div class="dialog-actions"><button class="secondary" id="plan-close">关闭</button>${p.can_apply?'<button class="primary" id="apply-plan">确认恢复</button>':''}</div>`);
 if(canBind)$('#bind-preview').onclick=()=>guarded(async()=>{await api('/bindings',{method:'POST',body:JSON.stringify({preview_id:j.id})});await refresh();$('#bind-preview').textContent='双向绑定已保存';$('#bind-preview').disabled=true;toast('下次恢复自动选中对应项目')});
...
document.addEventListener('click',e=>{
 ...
 case'bind-project':bindingModal(b.dataset.device,b.dataset.project);break;
 case'unbind-project':{const binding=(state.bindings||[]).find(x=>x.id===b.dataset.id);if(!binding)break;$('#modal-body').innerHTML=`<h2>解除项目绑定？</h2><p>${esc(deviceName(binding.a.device_id))} · ${esc(binding.a.path)}<br>↔ ${esc(deviceName(binding.b.device_id))} · ${esc(binding.b.path)}</p><p class="muted">只移除自动配对，不删除目录、会话或备份，也不会改变已排队的任务。</p><div class="dialog-actions"><button class="secondary" id="unbind-cancel">取消</button><button class="primary" id="unbind-confirm">确认解除</button></div>`;$('#unbind-cancel').onclick=closeModal;$('#unbind-confirm').onclick=()=>guarded(async()=>{await api('/bindings/'+binding.id,{method:'DELETE'});closeModal();await refresh();toast('已解除绑定')});break;}
