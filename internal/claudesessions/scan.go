...
func Scan(home claudehome.Home, _ ScanOptions) (sessions.ScanResult, error) {
	...
	// Relay: child transcripts may omit cwd; inherit only from their exact parent.
	parents := map[string]string{}
	for _, s := range result.Sessions {
		parents[s.RelPath] = s.CWD
	}
	for i := range result.Sessions {
		s := &result.Sessions[i]
		parts := strings.Split(s.RelPath, "/")
		if s.CWD == "" && len(parts) == 4 && parts[2] == "subagents" {
			s.CWD = parents[parts[0]+"/"+parts[1]+".jsonl"]
		}
	}
