...
- 0.3 限制：单会话（含 zstd 展开后）和单个关联文件最多 2 GiB，每个压缩包 2 GiB，展开总量 8 GiB，每次恢复最多 5000 个文件；服务器快照存储 20 GiB。JSONL 逐条处理，单条记录仍设 128 MiB 上限；这与整个会话文件的 2 GiB 上限不同。
...
- 0.3 Windows 二进制经过交叉编译，但尚未在 Windows 实机上完成新版真实续聊验收。Claude 的真实客户端续聊也尚未实测。具体测试结果见 `VALIDATION.md`。
