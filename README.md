...
1. 打开「我的设备」，搜索项目，点击项目下面的「绑定另一台电脑的项目」。
2. 选择对方设备及完整项目路径，核对后点击「确认并保存绑定」。同名目录只作为候选排在前面，不会自动建立绑定，也不扫描 Git 仓库来决定绑定。
3. 后续从任意一端创建快照、恢复到另一端时，页面会根据保存的绑定自动选择对应设备和项目，并明确显示已应用绑定。每次仍需要生成预览并确认恢复，可以手动选择其他目标，本次手动选择不更改原绑定。
4. 目标没有项目时，先按原流程新建并生成恢复预览；在预览页点击「确认两端是同一项目，保存绑定」。临时服务器包已经清理也可以绑定，来源元数据和目标本机预览仍然存在。
5. 项目下面的「管理绑定」显示双端目录，并允许明确确认后解除。解除不删除目录、会话或备份，也不改变已排队任务。

绑定保存在服务器 `state.json`，升级和重启后保留。绑定以设备身份、项目标识和确认时的路径为依据，同一项目在同一台对方设备上只能有一个对应项目；冲突需要先解除。可以分别绑定第三台设备，但不推断间接关系。目录不存在或路径变化时不会自动应用失效绑定。未绑定的项目继续使用原来的手动选择流程。

本功能只需升级服务器，已有 0.3 / 0.4 客户端可继续使用。恢复本身仍遵循各快照的最低客户端版本要求。绑定不会自动同步会话或代码，也不会把 Codex 和 Claude 的格式互相转换。
